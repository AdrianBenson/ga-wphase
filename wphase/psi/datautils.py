...
def rot_12_NE(st, META):
    ...
    for tr in st2:
        ...
        azi = META[id1]["azimuth"] * np.pi / 180.0
        C1 = tr1.data.copy()
        C2 = tr2.data.copy()
...
def PAZfromBLK(sp):
    ...
    for i in range(len(blk[53])):
        ...
        Empty = (
            blk[53][i].number_of_complex_poles == 0
            and blk[53][i].number_of_complex_zeros == 0
        )
...
def getCOEFF(respfile):
    ...
    h = np.real((-poles[0] - poles[1]) / 2.0 / om0)

    # That's wrong! In case TF=B, one has to divide omega by 2*pi
    trans, Resp = testRESP(respfile)
    if trans == "B":
        om0 = om0 * 2.0 * np.pi  # transforming hz to rad*hz
    if h > 0.9:
        G, om0, h = np.NaN, np.NaN, np.NaN
    ...
    for net in netlist:
        ...
        for st in stlist:
            ...
            for ch in chlist:
                ...
                tf = ch.getElementsByTagName("PzTransferFunctionType")[
                    0
                ].firstChild.data
                ...
                gain = float(
                    ch.getElementsByTagName("NormalizationFactor")[0].firstChild.data
                )
                sens = float(
                    ch.getElementsByTagName("InstrumentSensitivity")[0]
...
def griddataNaN(x, y, z, XX, YY, fill_value=np.nan):
    ...
    for i, coord in enumerate(coords):
        ZZ[
            np.unravel_index(np.where(np.all(coord == grid_coord, axis=1)), XX.shape)
        ] = z[i]
