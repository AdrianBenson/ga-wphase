...
def topography_colormap(cpool_sea, cpool_land):
    ...
    color_map = OrderedDict(
        [("red", initval[:]), ("green", initval[:]), ("blue", initval[:])]
    )

    nodes_sea = np.linspace(0.0, 0.5, n_col_sea)
...
def get_topography_cpool():
    cpool_sea = np.array(
        [[52, 61, 200], [0, 0, 127], [0, 0, 255], [102, 205, 200], [180, 255, 212]],
        dtype="float",
    )
...
def get_topography_slice(
    ...
    nlats = dim[1]
    nlons = dim[0]
    dlons = (xran[1] - xran[0]) / (nlons - 1.0)
    dlats = (yran[1] - yran[0]) / (nlats - 1.0)
    ...
    lats = np.linspace(
        yran[0] + latminind * dlats, yran[0] + latmaxind * dlats, lat_inter
    )
    lons = np.linspace(
        xran[0] + lonminind * dlons, xran[0] + lonmaxind * dlons, lon_inter
    )
    elev = np.empty((int(lat_inter + 1), int(lon_inter)))
    for i, latind in enumerate(
        np.arange(nlats - latminind - 2, nlats - latmaxind - 3, -1)
    ):
...
def plot_field(
    ...
    **kwargs,
    ...
    if plot_type == "scatter":
        ...
        grid_inter = griddata(
            (lats, lons), field, (grid_lats, grid_lons), method="linear"
        )
        ...
        field_plot = m.contour(
            grid_lons, grid_lats, grid_inter, clevs, latlon=True, **kwargs
        )
    output_dic = {
        "fig": fig,
        "ax": ax,
...
def stacov(
    ...
    cov_map.drawmeridians(
        np.arange(0, 360, 30), linewidth=0.4, color="grey", dashes=[5, 5]
    )
    cov_map.drawparallels(
        np.arange(-90, 90, 30), linewidth=0.4, color="grey", dashes=[5, 5]
    )
    ...
    if trlist is not None:
        ...
        for i in range(len(stnames)):
            plt.text(
                x[i], y[i], stnames[i], va="top", family="monospace", weight="bold"
            )
    if filename:
