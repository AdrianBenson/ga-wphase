...
def hypo2dist(hyploc1, hyploc2, r=6371.0):
    """
    ...
    theta = torad * locations2degrees(hyploc1[0], hyploc1[1], hyploc2[0], hyploc2[1])
...
def GetVirtualNetworkStations(VN):
    ...
    url = (
        "http://service.iris.edu/irisws/virtualnetwork/1/query?code=_"
        + VN
        + "&format=CSV"
    )
...
def resample_Ntraces(Ntraces, DecFac):
    ...
    for i in range(len(Ntraces)):
        if not i == 0:
            res = round((mat.ceil(new_val / DecFac) - new_val / DecFac) * DecFac)
...
def station_pruning(META, trlist, cutoffs=[1.0, 2.0, 5], units="deg"):
    """
        ...
        for i_trid, trid1 in enumerate(trlist[:-1]):
            for j_trid, trid2 in enumerate(trlist[i_trid + 1 :]):
                ...
                if units == "deg":
                    ...
                    dist = 1.0e-3 * gps2dist_azimuth(lat1, lon1, lat2, lon2)[0]
...
def get_new_coordinates(lat1, lon1, azi, dist):
    ...
    r = 6371.0
    dist_rad = np.pi / 180.0 * dist
    azi_rad = np.pi / 180.0 * azi
    lat1_rad = np.pi / 180.0 * lat1
    lon1_rad = np.pi / 180.0 * lon1
