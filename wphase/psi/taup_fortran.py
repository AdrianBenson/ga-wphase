...
def getTravelTimes(delta, depth, model="iasp91"):
    ...
    if not os.path.exists(model_path + os.path.extsep + "hed") or not os.path.exists(
        model_path + os.path.extsep + "tbl"
    ):
    ...
    for _i, phase in enumerate(phase_names):
        ...
        phase_name = phase.tostring().strip().replace(b"\x00", b"").decode()
        if not phase_name:
            break
        time_dict = {
